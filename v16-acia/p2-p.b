...
; added ACIA / RS232 Test 1.6 (from CBM2-Diagcart)
!cpu 6502
...
; ACIA register
DRSN			= $0 *2		; data register
SRSN			= $1 *2		; status register
CDR			= $2 *2		; command register
CTR			= $3 *2		; control register
; SID register
...
!addr acia		= $92 ; -$99	; ACIA register table
...
Main:	
	...
	jsr ACIATest			; acia test				********** EXTENDED **********
...
ACIATest:
	ldx #13				; "ACIA TEST"
	jsr PrintMessage		; print message
	jsr InitACIAPointer		; init cia pointer
	lda #SYSTEMBANK
	...
	sta temp3
rs232lp:lda (acia+CTR),y
	and #$f0			; clear baud rate bits
	ora temp3			; set baud rate
	sta (acia+CTR),y
	ldy temp3
	lda TestValuesRS232,y		; load test byte from table
	ldy #$00
	sta (acia+DRSN),y		; send
	lda (acia+CDR),y
	and #$f7			; enable transmit interrupt
	sta (acia+CDR),y
rsreclp:lda (acia+SRSN),y		; load status
	and #$08			; isolate transmit data register full bit
	beq rsreclp			; wait for full transmit
	lda (acia+DRSN),y
	ldy temp3
	cmp TestValuesRS232,y		; check received data
...
aciabad:lda #$35
	...
	lda #$d3
	sta pointer2+1			; set screen pointer to color RAM
	jsr ColorFaultyChip		; color 6551 U19 - V=0 if already colored
	jmp endacia
; ----------------------------------------------------------------------------
; rs232 test value table
TestValuesRS232:	!byte $ff, $55, $aa, $00, $01, $02, $04, $08
...
-	lda Messages,x
	...
	bne -
	cpx #14*9+9			; clear bank at sid test
...
;	!scr "         58 59 60 61 62 63 64 65 76 82  "
	...
	!scr "         ",$4c,$7a," ",$4c,$7a," ",$4c,$7a," ",$4c,$7a," ",$4c,$7a," ",$4c,$7a," ",$4c,$7a," ",$4c,$7a," ",$4c,$7a," ",$4c,$7a,"  "
	!scr "v. 1.5   82 83 84 04 19    02    24 85  "
